...
def solve_committor_nd_tt(
    per_dim:      Optional[List[PerDimMatrices]] = None,
    basis:        Optional[TensorProductBasis] = None,
    rho:          float = 400.0,
    density_spec: Optional[TTDensitySpec] = None,
    wA_fns:       Optional[List[ScalarFn]] = None,
    wB_fns:       Optional[List[ScalarFn]] = None,
    nquad:        int = 500,
    tt_rank:      int               = 4,
    n_sweeps:     int               = 20,
    tol:          float             = 1e-8,
    ...
    Mode 1: Rank-1 product density (original)
        Assumes a rank-1 product-measure for p, pA, pB (encoded in per_dim).
        
    Mode 2: General TT-format density (new)
        Uses a TTDensitySpec for a general low-rank non-separable density.
        For the Ginzburg-Landau problem with a non-product density, use this mode.

    Parameters
    ----------
    per_dim  : list of d PerDimMatrices, output of quadrature_matrices_nd.
               Required for Mode 1 (product density).
    basis    : TensorProductBasis — must match len(per_dim) and ns.
               Required for both modes.
    rho      : soft-boundary penalty weight.
    density_spec : Optional[TTDensitySpec]
               If provided, activates Mode 2 (general TT density).
               If None, uses Mode 1 (product density with per_dim).
    wA_fns   : length-d list of wA functions. Required for Mode 2.
    wB_fns   : length-d list of wB functions. Required for Mode 2.
    nquad    : quadrature points per site (used in Mode 2).
    ...
    if density_spec is not None:
        # Mode 2: General TT density
        if wA_fns is None or wB_fns is None:
            raise ValueError(
                "For TT-density mode, wA_fns and wB_fns must be provided."
            )
        if basis is None:
            # Reconstruct basis from density_spec.density_bases
            basis = TensorProductBasis(tuple(density_spec.density_bases))
        
        d = density_spec.d
        if d != basis.d:
            raise ValueError(
                f"density_spec.d={d} != basis.d={basis.d}"
            )
        
        # Use TT density assembly path
        hb_tt = assemble_hb_tt_density(basis, wB_fns, nquad)
    else:
        # Mode 1: Product density (original)
        if per_dim is None:
            raise ValueError(
                "For product-density mode, per_dim must be provided. "
                "For TT densities, provide density_spec instead."
            )
        if basis is None:
            raise ValueError("basis must be provided.")
        
        d = len(per_dim)
        if d != basis.d:
            raise ValueError(f"len(per_dim)={d} != basis.d={basis.d}.")
        
        # Use rank-1 (product) assembly path
        hb_tt = assemble_hb_tt(per_dim)

    # rho continuation schedule (same for both modes)
    ...
    if density_spec is None:
        # Only warn for rank-1 mode (this is the original behavior)
        warnings.warn(
    ...
    for stage_idx, rho_i in enumerate(rho_schedule):
        # Assemble MPO based on density mode
        if density_spec is not None:
            # TT density mode
            mpo_i = assemble_mpo_tt_density(
                density_spec, basis, wA_fns, wB_fns, rho_i, nquad
            )
        else:
