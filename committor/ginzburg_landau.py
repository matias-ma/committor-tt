...
def _gaussian_cheb_cores(
    ...
    for k in range(d):
        ...
        # Use Gauss-Chebyshev nodes of the first kind to compute the true
        # Chebyshev expansion coefficients.  At these nodes t_j = cos(theta_j)
        # with theta_j = (2j+1)*pi/(2*(N+1)), the Chebyshev orthogonality
        # weight 1/sqrt(1-t^2) is absorbed into the uniform quadrature weights
        # pi/(N+1), giving exact coefficients for polynomials up to degree N.
        n_cheb = N + 1   # number of Gauss-Chebyshev nodes
        j_arr   = np.arange(n_cheb)
        theta   = (2 * j_arr + 1) * np.pi / (2 * n_cheb)
        t_cheb  = np.cos(theta)                    # Chebyshev nodes on [-1, 1]
        xs_cheb = R_k * t_cheb + mid               # mapped to [a_k, b_k]

        # Chebyshev polynomials at the nodes (recurrence)
        T = np.empty((N + 1, n_cheb))
        T[0] = 1.0
        if N >= 1:
            T[1] = t_cheb
        for n in range(2, N + 1):
            T[n] = 2.0 * t_cheb * T[n - 1] - T[n - 2]

        # Gaussian values at Gauss-Chebyshev nodes
        pA_vals = np.exp(-0.5 * ((xs_cheb - mu[k]) / sigma) ** 2)

        # True Chebyshev expansion coefficients via Gauss-Chebyshev quadrature:
        #   c_0 = (1/n_cheb) * sum_j pA(x_j) * T_0(t_j)
        #   c_n = (2/n_cheb) * sum_j pA(x_j) * T_n(t_j)   for n >= 1
        # No Jacobian R_k needed here because the Chebyshev expansion is in t
        # (the physical-variable factor is absorbed into the evaluation basis).
        scale = np.full(N + 1, 2.0 / n_cheb)
        scale[0] = 1.0 / n_cheb
        c = scale * (T @ pA_vals)                  # shape (N+1,)
